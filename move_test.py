...
def vincenty_inverse(drone,ellipsoid=None):
    ...
    # 差異が無ければ0.0を返す
    if abs(start_lat-lat1)<0.000001 and abs(start_lon-lon1)<0.000001:
        s=0.0, α1=0.0, α2=0.0,
    else:
...
def moveby_dpi(drone):
    ...
    azimuth_rad = math.radians(β1+90)
    drone(
        extended_move_by(0, 0, -1, azimuth_rad, 0, 1, 1)
        >> FlyingStateChanged(state="hovering", _timeout=5)
    ).wait().success()
