...
def vincenty_inverse(ellipsoid=None):
    ...
    # 差異が無ければ0を返す
    if abs(lat0-lat1)<0.000001 and abs(lon0-lon1)<0.000001:
        s, α1, α2=0, 0, 0
        return (s, α1, α2)
    else:
...
def moveby_dpi(drone):
    s, α1, α2 = vincenty_inverse(ellipsoid=1)
    z0=-z+1
    azimuth_rad = α1+(pi/2)
    if 2*pi<=azimuth_rad:
        azimuth_rad1=azimuth_rad-2*pi
    else:
        if pi<azimuth_rad<2*pi:
            azimuth_rad1=-(2*pi-azimuth_rad)
        else:
            azimuth_rad1=azimuth_rad 
    print('---------------radians is '+str(azimuth_rad1)+' ---------------')
    drone(
